...
def C_BT(n0, uz0, rp, Tp):
    Cbt_1 = (mu0 * e**2) / (16 * kB) 
    Cbt_2 = n0 * uz0**2 * rp**3 / Tp
    return Cbt_1 * Cbt_2

def p0_CUBIC(n0, uz0, rp, Tp):
    '''
...
def BMAX(n0, uz0, rp, Tp):
    '''
    Calculate the maximum azimuthal magnetic field
    '''
    Cbt = C_BT(n0, uz0, rp, Tp)
    return mu0 * e * n0 * uz0 * f(n0, uz0, rp, Tp) / (2 * rp * (rp + Cbt))
...
def TAU_CONFINEMENT(n0, Tp, rp, p0, Bmax):
    '''
...
def P_ELECTRICAL(n0, uz0, rp, Tp):
    ...
    coeff2 = L / rp**4
    coeff3 = f(n0, uz0, rp, Tp)**2 / (Cbt + rp)**2
