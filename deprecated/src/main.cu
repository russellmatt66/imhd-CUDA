...
int main(int argc, char* argv[]){
	...
	InitializeIntAndSwap<<<execution_grid_dimensions, block_dims_init>>>(fluidvar_np1, intvar, Nx, Ny, Nz); // All 0.0
	checkCuda(cudaDeviceSynchronize());

	ComputeIntermediateVariables<<<execution_grid_dimensions, block_dims_intvar>>>(fluidvar, intvar, dt, dx, dy, dz, Nx, Ny, Nz);
	ComputeIntermediateVariablesBoundary<<<execution_grid_dimensions, block_dims_intvar>>>(fluidvar, intvar, dt, dx, dy, dz, Nx, Ny, Nz);
	checkCuda(cudaDeviceSynchronize());
	...
	for (size_t it = 0; it < Nt; it++){
		...
		FluidAdvance<<<execution_grid_dimensions, block_dims_fluid>>>(fluidvar_np1, fluidvar, intvar, D, dt, dx, dy, dz, Nx, Ny, Nz);
		BoundaryConditions<<<execution_grid_dimensions, block_dims_fluid>>>(fluidvar_np1, fluidvar, intvar, D, dt, dx, dy, dz, Nx, Ny, Nz);
		...
		SwapSimData<<<execution_grid_dimensions, block_dims_intvar>>>(fluidvar, fluidvar_np1, Nx, Ny, Nz);
		ComputeIntermediateVariables<<<execution_grid_dimensions, block_dims_intvar>>>(fluidvar_np1, intvar, dt, dx, dy, dz, Nx, Ny, Nz); // Avoid race condition w/int. vars
		ComputeIntermediateVariablesBoundary<<<execution_grid_dimensions, block_dims_intvar>>>(fluidvar_np1, intvar, dt, dx, dy, dz, Nx, Ny, Nz);
